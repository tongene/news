...
const Health = ({health_news }:{health_news:TopNews[]}) => { 
  const [posts, setPosts]=useState<PostProps[]>([]) 
  const [currPg, setCurrPg]=useState(1)
  const [postPerPage, setPostPerP]=useState(10) 
     const world_news = health_news.map((ex)=>ex.healthCategories?.nodes.filter((xy)=> xy?.name==="World")).flat().map((tx)=> tx?.healths.nodes).flat()
     const africa_news = health_news.map((ex)=>ex.healthCategories?.nodes.filter((xy)=> xy?.name==="Africa")).flat().map((tx)=> tx?.healths.nodes).flat()     
     const health_items=health_news.map((ex)=>ex.healthCategories?.nodes.filter((xy)=> xy?.name!=="World")).flat().filter((xy)=> xy?.name!=="Africa").map((tx)=> tx?.healths.nodes).flat()
  function decrement() {
   setCurrPg(currPg - 1);
 }
 function increment() {
