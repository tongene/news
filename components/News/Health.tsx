...
const Health = ({health_news }:{health_news:TopNews[]}) => { 
  ...
  const [postPerPage, setPostPerP]=useState(10)  
     const world_news = health_news.filter((ex)=>ex.healthCategories?.nodes[0].name==="World")
     const africa_news = health_news.filter((ex)=>ex.healthCategories?.nodes[0].name==="Africa") 
     const health_items=health_news.filter((ex)=>ex.healthCategories?.nodes[0].name!=="World").filter((ex)=>ex.healthCategories?.nodes[0].name!=="Africa")
  function decrement() {
   setCurrPg(currPg - 1);
 }
 function increment() {
