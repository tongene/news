...
const Society = ({society_news }:{society_news:TopNews[]}) => {

  const [posts, setPosts]=useState<PostProps[]>([]) 
  const [currPg, setCurrPg]=useState(1)
  const [postPerPage, setPostPerP]=useState(10) 
  
     const world_news = society_news.filter((ex)=>ex.societyCategories?.nodes[0].name==="World")
     const africa_news = society_news.filter((ex)=>ex.societyCategories?.nodes[0].name==="Africa") 
     const society_items=society_news.filter((ex)=>ex.societyCategories?.nodes[0].name!=="World").filter((ex)=>ex.societyCategories?.nodes[0].name!=="Africa")
...
<div className="max-w-2xl m-auto xl:m-0">
  <SlideFxn title_item={title_item} content={society_items.slice(0,4)}/>  
</div>
...
/> 
<Link href={`/news/society/${xy?.slug}`}><div dangerouslySetInnerHTML={{__html:xy?.excerpt}} style={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}className="overflow-hidden text-ellipsis my-2 text-lg hover:text-gray-500 py-2 leading-9"/></Link>
 <small className="text-sm my-3 text-red-500"><em>{moment(xy?.date).fromNow()}</em></small> 
</div>
)}
</div>
