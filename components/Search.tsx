...
xtcontent?.filter((vx)=> vx !==undefined).filter((ox)=> ox.contentTypeName!=="outline")?.map((it, index)=> it?.contentTypeName !=='naija-wiki' && it.contentTypeName !=='netflix-naija' && it.contentTypeName !=='post' && it.contentTypeName !=='page' &&           
...
{/* {name &&xtcontent?.length>0&&
xtcontent?.filter((vx)=> vx !==undefined)?.filter((ox)=> ox.contentTypeName!=="outline")?.map((it, index)=> it.contentTypeName ==='naija-wiki' &&               
<div key={it.id + Math.random()} className="shadow bg-white dark:bg-transparent max-w-xl px-4 m-auto my-2"> 
<div className="flex items-center">        
     <div className="max-w-32 xs:max-w-44"> 
     <Image
