...
xtcontent?.filter((vx)=> vx !==undefined)?.flat()?.map((it, index)=>
 it?.contentTypeName ==='netflix-naija'&&it?.netflixCategories?.nodes.length>0? it?.netflixCategories?.nodes.map((tx)=> tx?.naijaOnNetflix.nodes?.flat()?.map((itx, index)=>   
...
<div className="flex justify-center xs:items-center">
 <div className="max-w-32 xs:max-w-44"> 
     ...
     width={1200}
     height={675}
     alt={itx?.featuredImage?.node.altText}
     />
     </div>
     <div className="mx-4 xs:py-5">            
...
<div className="flex justify-center xs:items-center">        
     ...
     <div className="mx-4 xs:py-5">            
 <Link href={`/news/video/${it?.slug}/`}><h3 className="hover:opacity-50 text-xl cursor-pointer font-medium leading-tight">{it?.title }</h3></Link> 
 <div className="flex flex-wrap pt-4" >
   {it?.contentTags?.nodes?.slice(0,5)?.map((ex, i)=>
 <div  key={i + Math.random()}>
  <Link href={`/topic/${ex?.slug}/`}>{ex&&<span className="cursor-pointer hover:opacity-50 mx-2">#{ex.name }  
...
<div className="flex justify-center xs:items-center"> 
     ...
     <div className="mx-4 xs:py-5">            
 <Link href={`/news/${it?.slug}/`}><h3 className="hover:opacity-50 text-xl cursor-pointer font-medium leading-8">{it?.title }</h3></Link> 
 <div className="flex flex-wrap pt-4" >
   {it?.contentTags?.nodes?.slice(0,5)?.map((ex, i)=>
 <div  key={i + Math.random()}>
  <Link href={`/topic/${ex?.slug}/`}>{ex&&<span className="cursor-pointer hover:opacity-50 mx-2">#{ex.name }  
