...
<p className="text-xl bg-gray-700 p-4 text-white  w-1/2 xs:w-1/3 md:w-1/2 lg:w-1/4">Year: </p><span className="text-lg w-3/4 p-4">{charactertitles.charactertitles.releaseDate}</span>
 ...
 <Link href={`/character/${xx?.slug}/`}><h3 className="text-xl font-bold p-3 text-center"> {xx?.title} </h3></Link> 
...
<div className="w-72 h-44">
  ...
  height={675}
   ...
   alt={xy.filmImg1.node.altText}/>
    ...
    <Link href={`/character/${xy.characterWiki.slice(0, -5).toLowerCase().replace(/ /g,'-').replace(/\./g, '')}/`}> <h3 className='text-gray-800 hover:text-gray-700 hover:dark:text-gray-500 dark:text-gray-300 font-bold hover:bg-red-500 hover:text-white cursor-pointer text-center overflow-hidden text-ellipsis py-3'>{xy.characterWiki}</h3></Link> 
</div>
)}
</div>  
</div>
