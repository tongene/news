...
const [end_inter_cursor, setEnd_inter_cursor] = useState('');
 ...
 <Link href={`/character/${it.slug}/`}><h3 className='text-white font-bold px-5 py-1 text-2xl'>{it.title }</h3></Link> 
  <span className='text-white ml-2'>by</span><Link href={`/character/${it.slug }/`}><p className='text-white hover:text-gray-200 font-bold py-1 px-5 mx-1 text-lg'> {it.charactertitles.portrayedby}</p></Link> 
 
 </div>
 <div className='text-white mx-4 pb-6 pt-3'>
 <Link href={`/character/${it.slug }/`}><div dangerouslySetInnerHTML={{__html:it.excerpt}} className='overflow-hidden text-ellipsis hover:text-gray-200 leading-relaxed text-lg cursor-pointer'style={{ display: '-webkit-box', WebkitLineClamp:3, WebkitBoxOrient: 'vertical' }} /></Link>   
 ...
 <section className='lg:flex relative m-auto ' style={{maxWidth:'1550px'}}> 
  ...
  <div key={nt.slug} className='border border-orange-400'>
    ...
    <Link href={`/netflix-naija/${nt.slug}/`}><h2 className='xs:text-2xl md:text-4xl py-2 hover:text-gray-400'> {nt.title}</h2></Link>
    <Link href={`/netflix-naija/${nt.slug}/`}><div dangerouslySetInnerHTML={{__html:nt.excerpt}} className='sm:text-lg py-2 hidden sm:block hover:text-gray-400'/> </Link> 
    <span className='text-end text-sm mt-11 italic'>{moment(nt.date).subtract(1, 'hour').fromNow()} </span>
    </div>
    </div> 
       )    
) }</div> 
...
<Link href={`/netflix-naija/${item.slug}/`}><h3 className='text-2xl py-3'>{item.title}</h3></Link>
 <Image
...
<Link href={`/netflix-naija/${item.slug }/`}><h3 className='text-2xl py-3 xl:py-0'>{item.title}</h3></Link>
 <Image
...
<Link href={`/topic/${item.contentTags.nodes[0].slug}/`}><h4 className=' py-1 font-bold'>&#8212; {item.contentTags.nodes[0].name }</h4></Link> 
 ...
 <ul className='list-none text-lg [&>*:nth-child(odd)]:bg-gray-500 [&>*:nth-child(even)]:bg-blue-200 [&>*:nth-child(odd)]:text-white [&>*:nth-child(even)]:text-black font-bold'>
  ...
  <ol className='flex p-8 justify-between'key={oneItem.id}> <Link href= {`/netflix-naija/${oneItem.slug}/` }><li className='mx-5 px-4 hover:opacity-70'>{oneItem.title} ...</li></Link></ol> 
