...
<section className='lg:flex relative '> 
  ...
  className="grid-img"
    ...
    <Link href={`/${slug.node.slug}/${nt.node.slug}/`}><h2 className='text-xl xs:text-3xl md:text-4xl md:py-2 hover:text-gray-500'> {nt.node.title}</h2></Link>
    <Link href={`/${slug.node.slug}/${nt.node.slug}/`}><div className='xs:text-lg text-sm py-2 xs:block hover:text-gray-500' dangerouslySetInnerHTML={{__html:nt.node.excerpt}}/>  </Link> 
...
<Link href='/coming-to-netflix/'><p className='p-2 font-bold text-center'>Coming to Netflix Naija</p></Link> 
</div> 
<hr/>
...
<NewsLetter/>
 ...
 <Link href={`/netflix-naija/${xy.slug}/`}><h2 className='hover:text-gray-400 text-gray-600 text-xl overflow-hidden text-ellipsis'style={{ display: '-webkit-box', WebkitLineClamp:3, WebkitBoxOrient: 'vertical' }}>{xy.title} </h2></Link>
  </div> 
 </div> 
 )}</div>
