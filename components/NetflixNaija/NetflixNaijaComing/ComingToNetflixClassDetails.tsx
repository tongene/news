...
import { faAngleRight, faClapperboard, faClock, faFilm, faUser } from "@fortawesome/free-solid-svg-icons";
...
endCursor:string
  ...
  const ComingToNetflixClassDetails = ({coming_to_netflix_naija, netflix_related, netflix_news, coming_to_netflix_details }:{coming_to_netflix_details:TitleProps, coming_to_netflix_naija:TitleProps[], netflix_related:TitleProps[], netflix_news:TitleProps[]}) => {  
...
/>   
  ...
  <Link href={`/coming-to-netflix/${xy.node.slug}/`}><h2 className='text-xl font-bold overflow-hidden text-ellipsis'style={{ display: '-webkit-box', WebkitLineClamp:2, WebkitBoxOrient: 'vertical' }}>{xy.node.title}</h2></Link> 
  ...
  <p className='mx-2 w-full'>{new Date(xy.node.date).toDateString()}</p>
   ...
   <Link href='/new-on-netflix/'><p className='p-3 font-bold text-center'>On Netflix Naija</p></Link> 
   </div>
   ...
   <Link href='/coming-to-netflix/'><p className='p-3 font-bold text-center'>Coming to Netflix Naija</p></Link>  
   </div> 
   <div className='py-5 text-gray-200 flex flex-col justify-center w-32 h-32 cursor-pointer bg-yellow-500 my-2 md:bg-transparent md:text-gray-600 md:rounded-none rounded-full'> 
   ...
   <Link href={`/netflix-naija/${xy.slug}/`}><h2 className='text-base hover:text-gray-400 text-gray-600'>{xy.title} </h2></Link>
    </div> 
   </div> 
   )}</div>
  ...
  <div className="max-w-7xl m-auto overflow-auto pt-4 px-1 hidden-scroll" >   
     ...
     {next_posts.slice(0,3).map((xy,i)=> 
      ...
      <Link href={`/coming-to-netflix/${xy.node.slug}/`}><h2 className="text-gray-800 hover:text-gray-700 text-base font-bold overflow-hidden text-ellipsis hover:text-gray-500 cursor-pointer"style={{ display: '-webkit-box', WebkitLineClamp:2, WebkitBoxOrient: 'vertical' }}>{xy.node.title}</h2></Link>            
